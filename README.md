#STM32F072-Discovery Application Template
This package is for use when compiling programs for STM32F072 ARM microcontrollers using arm-none-eabi-gcc. The Makefile in the main directory will call the Make file in the Drivers directory, thereby automatically building the STM32 Cube library. However, running 'make clean' will not affect the peripherals library (the same command can be run from the Drivers directory to do this).
...
1. Drivers/
   * This is the Drivers/ folder from the STM32Cube_FW_F0_V1.5.0 standard peripheral driver library produced by STM. This preserves the original structure which should make it easy to roll in library upgrades as they are published
   * **Makefile** is not part of the STM release, and must be copied over if the library is upgraded.
   * **choose_your_mcu_here.h** is not part of the STM release, and must be copied over if the library is upgraded. Additionally your MCU should be chosen here.
   * **stm32f0xx_hal_conf.h** is used to configure the peripheral library. It must be copied here if the library is upgraded. The file was file taken from the STM32Cube_FW_F0_V1.5.0 firmware package. It is found in the following directory:
      * Projects/STM32072B_EVAL/Templates/Inc
   * **Abstracting the libraries:** You may place this folder anywhere you like in order to use it for multiple projects. Just change the path of the CUBE_DRIVERS_DIR variable in the Makefile
...
4. src/
   ...
   * All files included in this repository are taken from the STM32Cube_FW_F0_V1.5.0 package. It is found in the following directory:
      * Projects/STM32072B_EVAL/Templates/Inc
